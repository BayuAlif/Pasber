...
import SidebarAdmin from "@/app/components/sidebar-admin/page";
...
export default function AdminRiwayatServicePage() {
  ...
  const uniqueVehicles = Array.from(new Set(records.map((r) => r.vehicle).filter((v) => v !== "-")));
  const filtered = records.filter((r) => {
    const term = search.toLowerCase().trim();
    const matchSearch =
      r.wo.toLowerCase().includes(term) ||
      r.vehicle.toLowerCase().includes(term) ||
      r.mechanic.toLowerCase().includes(term) ||
  ...
  return (
    <div className="flex min-h-screen bg-[#0f1117] text-[#e2e8f0]" style={{ fontFamily: "'DM Sans','Segoe UI',sans-serif" }}>
      ...
      <main className="ml-[220px] flex-1 px-9 py-8">
        <div className="flex justify-between items-start mb-7">
          <div>
            ...
            <p className="text-[13px] text-[#6b7280] mt-1 mb-0">
              Semua riwayat servis pelanggan tersimpan di sini.
            </p>
          </div>
        ...
        <div className="bg-[#13161e] border border-[#1e2230] rounded-xl overflow-hidden mb-4">
          <div
            className="grid px-5 py-3 border-b border-[#1e2230] bg-[#0f1117]"
          ...
          >
            {['NO. WO', 'KENDARAAN', 'JENIS SERVICE', 'TANGGAL', 'MEKANIK', 'TOTAL', 'STATUS', 'AKSI'].map((h) => (
              <div key={h} className="text-[10px] font-bold text-[#4b5563] tracking-[1.2px]">
                {h}
              </div>
            ))}
          </div>
          ...
          {loading ? (
            <div className="py-12 px-5 text-center text-[#4b5563]">
              ...
              <p className="text-[13px]">Memuat riwayat servis...</p>
            </div>
          ) : paginated.length === 0 ? (
            <div className="py-12 px-5 text-center text-[#4b5563]">
              <History size={32} className="mx-auto mb-2.5 opacity-40" />
              ...
              <button
                ...
                className={`w-[30px] h-[30px] rounded-[7px] border text-[12px] font-bold cursor-pointer ${
                  page === i + 1
                    ...
                    : "bg-[#1a1d28] border-[#2a2f3e] text-[#6b7280]"
                }`}
              >
                {i + 1}
              </button>
        ...
        >
          ...
          >
            ...
            <div className="flex gap-2.5">
              {detailRow.status === "running" && (
                <Link href="/Admin/jadwal" className="flex-1 no-underline">
